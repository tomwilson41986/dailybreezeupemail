...
1. **Discover sales**: fetch `https://www.racingpost.com/bloodstock/sales/catalogues/` and filter to sale records whose name matches `/breeze.?up/i` in the current calendar year, plus the Tattersalls Guineas sale (which runs the day after Craven and re-offers the unsold-at-Craven 2yos under Tatts' internal `breezeup2` catalogue). The Guineas sale is matched on "Tattersalls Guineas" alone — RP listed it as "Tattersalls Guineas Horses-in-Training Sale 2026" before the sale and renamed the record to "Tattersalls Guineas Sale 2026" once it was past, and anchoring on the longer name silently dropped it (and every graduate with it) from May 2026's emails. The sale also contains older HIT lots — `fetch_lots` filters those out, keeping only age-2 rows. As a second guard against renamed/removed index records, the hardcoded known-sale list (`_BREEZE_UP_FALLBACK`, also the fallback when the index page is blocked) is merged into discovery as a floor, keyed on `(venue_uid, sale_date)`; a sale restored this way logs a warning. Typical 2026 set: Tattersalls Craven, Goffs UK 2yo Breeze Up, Tattersalls Guineas (2yos only), Arqana May 2yo Breeze Up, Osarus Breeze-Up & HIT, Tattersalls Ireland Breeze Up, Aktem Paris Breeze-Up, Goffs Classic.
