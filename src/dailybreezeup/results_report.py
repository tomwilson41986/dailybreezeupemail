"""Racing-results workbook for the gSheet breeze-up cohort.

Shared by ``scripts/racing_results_xlsx.py`` (one-shot CLI) and the Friday
weekly summary email (``daily --mode weekly``), which attaches the workbook.

The workbook joins every sheet row to RP's sale catalogues on (sale, lot),
then to each named horse's form pulled from the RP profile-tab endpoint
(``/profile/tab/horse/<uid>/x/form``; the slug segment is ignored by RP and
``rpPostmark`` is the RPR). Sheets:
...
openpyxl is an optional dependency (``pip install -e ".[xlsx]"``); it is
imported lazily so the daily morning/evening modes never need it.
"""
...
from dailybreezeup.racing.rp_sales import _XHR_HEADERS, BASE
...
def lot_lookup(lots: Iterable[rp_sales.SaleLot]) -> dict[tuple[str, str], rp_sales.SaleLot]:
    """Index catalogue lots by the sheet join key ``(sale label, lot string)``."""
    out: dict[tuple[str, str], rp_sales.SaleLot] = {}
    for lot in lots:
        label = sheet_label_for_sale(lot.sale.sale_name)
        if label is not None:
            out[(label, lot.display_lot)] = lot
    return out
...
def _get(session: Any, url: str, referer: str, attempts: int = 4) -> str | None:
...
def fetch_form_stats(session: Any, uid: int, *, sleep: float = 0.45) -> HorseStats | None:
...
def collect_form_stats(
    lots: Iterable[rp_sales.SaleLot],
    session: Any,
    *,
    sleep: float = 0.45,
) -> dict[int, HorseStats]:
    """Fetch form stats for every distinct named horse across ``lots``."""
    uids = sorted({lot.horse_uid for lot in lots if lot.horse_uid})
    log.info("fetching form for %d named horses", len(uids))
    stats: dict[int, HorseStats] = {}
    for i, uid in enumerate(uids, 1):
        s = fetch_form_stats(session, uid, sleep=sleep)
        if s is not None:
            stats[uid] = s
        if i % 50 == 0:
...
def build_workbook_bytes(
    ...
    per_band: dict[str, dict[str, Any]] = {
        b: {"lots": 0, "raced": 0, "ftos": [], "peaks": []} for b in BAND_ORDER
    }
    for row in rows:
